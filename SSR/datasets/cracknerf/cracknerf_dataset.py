...
class CrackNerfData(Dataset):
    def __init__(self, data_dir, train_ids, test_ids, img_h=None, img_w=None):
        ...
        self.rgb_list = sorted(glob.glob(self.rgb_dir + '/rgb*.png'),
                               key=lambda file_name: int(file_name.split("_")[-1][:-4]))
        self.depth_list = sorted(glob.glob(self.depth_dir + '/depth*.png'),
                                 key=lambda file_name: int(file_name.split("_")[-1][:-4]))
        self.semantic_list = sorted(glob.glob(self.semantic_class_dir + '/semantic_class_*.png'),
                                    key=lambda file_name: int(file_name.split("_")[-1][:-4]))
        if self.semantic_instance_dir is not None:
            self.instance_list = sorted(glob.glob(self.semantic_instance_dir + '/semantic_instance_*.png'),
                                        key=lambda file_name: int(file_name.split("_")[-1][:-4]))
        ...
        for idx in train_ids:
            ...
            depth = cv2.imread(self.depth_list[idx],
                               cv2.IMREAD_UNCHANGED) / 1.0
          #  depth = cv2.imread(self.depth_list[idx],
          #                     cv2.IMREAD_UNCHANGED) / 1000.0  # uint16 mm depth, then turn depth from mm to meter
        ...
        for idx in test_ids:
            image = cv2.imread(self.rgb_list[idx])[:, :, ::-1] / 255.0  # change from BGR uinit 8 to RGB float
            depth = cv2.imread(self.depth_list[idx],
                               cv2.IMREAD_UNCHANGED) / 1.0 # uint16 mm depth, then turn depth from mm to meter
            semantic = cv2.imread(self.semantic_list[idx], cv2.IMREAD_UNCHANGED)
            if self.semantic_instance_dir is not None:
                instance = cv2.imread(self.instance_list[idx], cv2.IMREAD_UNCHANGED)  # uint16
        ...
        self.num_semantic_class = self.semantic_classes.shape[
            0]  # number of semantic classes, including the void class of 0
        ...
        for i in range(self.num_semantic_class):
            ...
            self.train_samples["semantic_remap_clean"][
                self.train_samples["semantic_clean"] == self.semantic_classes[i]] = i
        ...
        for key in self.train_samples.keys():
            print("{} has shape of {}, type {}.".format(key, self.train_samples[key].shape,
                                                        self.train_samples[key].dtype))
        print()
        print("Testing Sample Summary:")
        for key in self.test_samples.keys():
            print(
                "{} has shape of {}, type {}.".format(key, self.test_samples[key].shape, self.test_samples[key].dtype))
    ...
    def add_pixel_wise_noise_label(self,
        ...
        if not load_saved:
            ...
            print("{} of {} semantic labels are added noise {} percent area ratio.".format(sum(self.mask_ids),
                                                                                           len(self.mask_ids),
                                                                                           noise_ratio))

            if visualise_save:
                noisy_sem_dir = os.path.join(self.semantic_class_dir,
                                             "noisy_pixel_sems_sr{}_nr{}".format(sparse_ratio, noise_ratio))
                if not os.path.exists(noisy_sem_dir):
                    os.makedirs(noisy_sem_dir)
                with open(os.path.join(noisy_sem_dir, "mask_ids.npy"), 'wb') as f:
                    np.save(f, self.mask_ids)
                ...
                for i in range(len(self.mask_ids)):
                    if self.mask_ids[i] == 1:
                        ...
                        imageio.imwrite(os.path.join(noisy_sem_dir, "semantic_class_{}.png".format(i)),
                                        semantic_remap[i])
                        imageio.imwrite(os.path.join(noisy_sem_dir, "vis_sem_class_{}.png".format(i)),
                                        vis_noisy_semantic)
            ...
            self.mask_ids = np.load(os.path.join(noisy_sem_dir, "mask_ids.npy"))
            semantic_img_list = []
            semantic_path_list = sorted(glob.glob(noisy_sem_dir + '/semantic_class_*.png'),
                                        key=lambda file_name: int(file_name.split("_")[-1][:-4]))
        ...
        if not load_saved:
            ...
            for img_idx in range(self.train_num):
                ...
                for ins_idx in instance_id:
                    ...
                    if instance_ratio > 0 and self.mask_ids[
                        img_idx] == 1:  # larger than 1% image area and the image is also sampled into training set
            ...
            for k, v in instance_maps_dict.items():
                instance_maps_dict[k] = sorted(instance_maps_dict[k],
                                               key=lambda x: x[1])  # sorted, default is ascending order
            if not uniform_flip:
                ...
                for i in range(len(instance_id)):  # loop over instance id
                    selected_frame_id = [x[0] for x in
                                         instance_maps_dict[instance_id[i]][:num_flip_frame_per_instance_id[i]]]
                    for m in selected_frame_id:  # loop over image ids having the selected instance
                        self.train_samples["semantic_remap"][m][
                            self.train_samples["instance"][m] == instance_id[i]] = np.random.choice(
            ...
            if visualise_save:
                ...
                for i in range(len(vis_flip_semantic)):
                    imageio.imwrite(os.path.join(flip_sem_dir, "semantic_class_{}.png".format(i)),
            ...
            semantic_img_list = []
            semantic_path_list = sorted(glob.glob(flip_sem_dir + '/semantic_class_*.png'),
                                        key=lambda file_name: int(file_name.split("_")[-1][:-4]))
            assert len(semantic_path_list) > 0
            for idx in range(len(self.mask_ids)):
                semantic = imread(semantic_path_list[idx])
    ...
    def super_resolve_label(self, down_scale_factor=8, dense_supervision=True):
        ...
        if dense_supervision:  # for dense labelling,  we down-scale and up-scale label maps again
            ...
            for i in range(self.train_num):
                ...
                scaled_low_res_label = cv2.resize(low_res_label, (self.img_w, self.img_h),
                                                  interpolation=cv2.INTER_NEAREST)
            ...
            self.train_samples["semantic_remap"] = (
                        self.train_samples["semantic_remap"] * valid_low_res_pixel_mask[None, ...]).astype(np.uint8)
    ...
    def simulate_user_click_partial(self, perc=0, load_saved=False, visualise_save=True):
        ...
        if not load_saved:
            ...
            if single_click:
                ...
                for i in range(self.train_num):
                    ...
                    for l in valid_class:
                        ...
                        target_num = int(
                            perc * label_mask_.sum())  # find the target and total number of pixels belong to class-l in the current image
                        label_mask[
                            label_idx_[0, 0], label_idx_[0, 1]] = 1  # full-zero mask with only selected pixel to be 1
            ...
            semantic_img_list = []
            semantic_path_list = sorted(glob.glob(saved_partial_sem_dir + '/semantic_class_*.png'),
                                        key=lambda file_name: int(file_name.split("_")[-1][:-4]))
            assert len(semantic_path_list) > 0
