...
class SSRTrainer(object):
    ...
    def prepare_data_cracknerf(self, data, gpu=True):
        self.ignore_label = -1
        ...
        total_num_classes = len(annotations["classes"]) + 1
        ...
        colour_map_np = label_colormap(total_num_classes)[
            data.semantic_classes]  # select the existing class from total colour map
        self.colour_map = torch.from_numpy(colour_map_np)
        self.valid_colour_map = torch.from_numpy(
            colour_map_np[1:, :])  # exclude the first colour map to colourise rendered segmentation without void index
        ...
        self.viz_train_depth = np.stack(
            [depth2rgb(dep, min_value=self.near, max_value=self.far) for dep in train_samples["depth"]],
            axis=0)  # [num_test, H, W, 3]
        ...
        self.viz_train_semantic = np.stack([colour_map_np[sem] for sem in self.train_semantic],
                                           axis=0)  # [num_test, H, W, 3]

        self.train_semantic_clean = torch.from_numpy(train_samples["semantic_remap_clean"])
        self.viz_train_semantic_clean = np.stack([colour_map_np[sem] for sem in self.train_semantic_clean],
                                                 axis=0)  # [num_test, H, W, 3]
        ...
        self.viz_test_depth = np.stack(
            [depth2rgb(dep, min_value=self.near, max_value=self.far) for dep in test_samples["depth"]],
            axis=0)  # [num_test, H, W, 3]
        # process the depth for evaluation purpose
        ...
        self.viz_test_semantic = np.stack([colour_map_np[sem] for sem in self.test_semantic],
                                          axis=0)  # [num_test, H, W, 3]
        ...
        self.tfb_viz.tb_writer.add_image('Train/vis_sem_label_GT_clean', self.viz_train_semantic_clean, 0,
                                         dataformats='NHWC')
    ...
    def step(
        ...
        if dataset_type == "cracknerf" or dataset_type == "replica" or dataset_type == "replica_nyu_cnn" or dataset_type == "scannet":
