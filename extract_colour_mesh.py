...
from SSR.datasets.cracknerf import cracknerf_dataset
...
def train():
    ...
    colour_map = torch.from_numpy(label_colour_map)
    valid_colour_map = torch.from_numpy(valid_colour_map)


    total_num = 113
    train_num = 100
    train_ids = list(range(train_num))
    test_ids = list(range(train_num, total_num))
    # add ids to config for later saving.
    config["experiment"]["train_ids"] = train_ids
    config["experiment"]["test_ids"] = test_ids

    cracknerf_data_loader = cracknerf_dataset.CrackNerfData(data_dir=config["experiment"]["dataset_dir"],
                                                            train_ids=train_ids, test_ids=test_ids,
                                                            img_h=config["experiment"]["height"],
                                                            img_w=config["experiment"]["width"])

    ssr_trainer.set_params_cracknerf()
    ssr_trainer.prepare_data_cracknerf(cracknerf_data_loader)
    ...
    train_Ts_np =  cracknerf_data_loader.train_samples["T_wc"]
    #mesh_file = os.path.join(mesh_dir,"mesh.ply")
    mesh_file = os.path.join(mesh_dir, "model_export.obj")
    ...
    with torch.no_grad():
        ...
        raw = torch.cat(
            [run_MLP_fn(grid_query_pts[i: i + chunk]).cpu() for i in range(0, grid_query_pts.shape[0], chunk)], dim=0)
        rgb = torch.sigmoid(raw[..., :3])  # [N_rays, N_samples, 3]
        alpha = raw[..., 3]  # [N]
        sem_logits = raw[..., 4:]  # [N_rays, N_samples, num_class]
        label_fine = logits_2_label(sem_logits).view(-1).cpu().numpy()

        # Ensure valid indices for label_colour_map
        label_fine_valid_indices = np.clip(label_fine + 1, 0, len(label_colour_map) - 1)
        vis_label_colour = label_colour_map[label_fine_valid_indices]
    ...
    v_colors = v_colors.cpu().numpy().astype(np.uint8)

    o3d_mesh_canonical_clean.vertex_colors = o3d.utility.Vector3dVector(v_colors/255.0)

    if args.sem:
        o3d.io.write_triangle_mesh(os.path.join(mesh_recon_save_dir, 'semantic_mesh_canonical_dim{}neart_{}.obj'.format(grid_dim, near_t)), o3d_mesh_canonical_clean)
        print("Saving Marching Cubes mesh to semantic_mesh_canonical_dim{}neart_{}.obj".format(grid_dim, near_t))
    else:
        o3d.io.write_triangle_mesh(os.path.join(mesh_recon_save_dir, 'colour_mesh_canonical_dim{}neart_{}.obj'.format(grid_dim, near_t)), o3d_mesh_canonical_clean)
        print("Saving Marching Cubes mesh to colour_mesh_canonical_dim{}neart_{}.obj".format(grid_dim, near_t))
